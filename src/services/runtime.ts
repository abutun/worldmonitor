...
export function getRemoteApiBaseUrl(): string {
  ...
  if (isDesktopRuntime()) return 'https://monitor.cosmicmeta.ai';
  return '';
}
...
function isAppOriginUrl(urlStr: string): boolean {
  try {
    ...
    return APP_HOSTS.has(host) || host.endsWith('.cosmicmeta.ai');
  } catch {
    return false;
  }
}
...
const ALLOWED_REDIRECT_HOSTS = /^https:\/\/([a-z0-9]([a-z0-9-]*[a-z0-9])?\.)*cosmicmeta\.ai(:\d+)?$/;
